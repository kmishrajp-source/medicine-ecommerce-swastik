...
export function detectIntent(msg) {
    ...
    if (/how.*search.*medicine|explain.*medicine.*search|find.*medicine/i.test(text) && /how|explain/i.test(text)) return "search_medicine_help";
    if (/how.*search.*doctor|explain.*doctor.*search|find.*doctor/i.test(text) && /how|explain/i.test(text)) return "search_doctor_help";
...
export async function processChatMessage(message) {
    try {
        ...
        // ── OpenAI Fallback for unrecognised queries ─────────────────────────
        // Only fires when the existing deterministic system cannot classify the message.
        // All known healthcare intents above are handled WITHOUT calling OpenAI (cost-efficient).
        if (intentKey === "unknown" && process.env.OPENAI_API_KEY) {
            try {
                console.log('[AI Brain] Unknown intent — attempting OpenAI fallback for:', message.substring(0, 60));
                ...
                const completion = await openai.chat.completions.create({
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: `You are Sofiya, a warm, knowledgeable AI healthcare assistant for Swastik Medicare (India).

Swastik Medicare is a digital healthcare platform offering: medicine delivery (Gorakhpur & Delhi NCR), doctor consultations, lab tests, ambulance services, health insurance guidance, and a pharmacy partner network.

**Your Responsibilities:**
1. For general knowledge questions (science, facts, education) — answer clearly and concisely in 3-5 sentences.
2. For healthcare / medical questions — provide helpful information but always recommend consulting a real doctor. Append: "\n\n⚠️ *This is for educational purposes only. Please consult a doctor for medical advice.*"
3. For questions about medicines — give general educational info only. Never invent drug prices, availability, or prescriptions.
4. For questions about Swastik services — guide users: medicine delivery at https://swastikmed.online/en/shop-medicines, doctors at https://swastikmed.online/en/doctors.
5. Always respond in the SAME LANGUAGE the user wrote in (Hindi, Hinglish, or English).
6. Keep responses concise and friendly. Use bullet points where helpful. Do not invent business data (prices, stock, order status).`
                        },
                ...
                if (aiText) {
                    console.log('[AI Brain] OpenAI fallback succeeded.');
                    return {
                        responseText: aiText,
                        sources: ['Swastik AI (OpenAI)'],
                        disclaimer: 'AI-generated response. For medical decisions, consult a qualified doctor.'
                    };
                }
            } catch (openaiErr) {
                // Classify the error safely — never expose the key
                const errMsg = openaiErr?.message || '';
                let errCategory = 'OPENAI_ERROR_UNKNOWN';
                if (errMsg.includes('401') || errMsg.includes('Incorrect API key') || errMsg.includes('invalid_api_key')) errCategory = 'OPENAI_ERROR_AUTH';
                else if (errMsg.includes('429') || errMsg.includes('rate_limit') || errMsg.includes('quota')) errCategory = 'OPENAI_ERROR_RATE_LIMIT';
                else if (errMsg.includes('timeout') || errMsg.includes('ETIMEDOUT') || errMsg.includes('ECONNREFUSED')) errCategory = 'OPENAI_ERROR_NETWORK';
                else if (errMsg.includes('503') || errMsg.includes('overloaded')) errCategory = 'OPENAI_ERROR_UNAVAILABLE';
                console.warn('[AI Brain] OpenAI fallback failed:', errCategory);
                // Fall through to existing RESPONSES.unknown below
            }
        } else if (intentKey === "unknown") {
            console.log('[AI Brain] OPENAI_API_KEY not set — using static fallback for unknown intent.');
        }
        // ─────────────────────────────────────────────────────────────────────
