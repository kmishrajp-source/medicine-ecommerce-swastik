...
export function detectIntent(msg) {
    ...
    const otcMatch = text.match(/\b(headache|fever|acidity|gas|heartburn|cold|cough|runny nose|sneeze|allergies|allergy)\b/i);
    if (otcMatch && !/doctor|hospital|clinic/i.test(text)) {
        let drugName = "paracetamol"; // default for fever/headache
        if (/acidity|gas|heartburn/i.test(otcMatch[0])) drugName = "pantoprazole";
        if (/cold|cough|runny nose|sneeze|allergies|allergy/i.test(otcMatch[0])) drugName = "cetirizine";
        return { type: "otc_symptom", symptom: otcMatch[0], drug: drugName };
    }
