...
export async function sendWhatsAppMessage(to, templateName, variables) {
    ...
    if (templateName === 'settlement_batch_created') {
        messageBody = `🏦 *Settlement Processing — Swastik Medicare*\n\nHello *${variables[0]}*,\n\nYour settlement batch *${variables[1]}* is now being processed.\n\n💰 *Net Payout:* ₹${variables[2]}\n📦 *Orders Included:* ${variables[3]}\n\nExpected transfer: Within 2 working days to your registered bank account.\n\n📊 View your Ledger: https://www.swastikmed.online/en/retailer/settlements`;
    }
    if (templateName === 'settlement_paid') {
        messageBody = `✅ *Payment Transferred — Swastik Medicare*\n\nHello *${variables[0]}*,\n\nYour settlement *${variables[1]}* has been successfully transferred!\n\n💰 *Amount:* ₹${variables[2]}\n🏦 *To:* Your registered bank account\n\n📊 View full ledger: https://www.swastikmed.online/en/retailer/settlements\n\nThank you for partnering with Swastik Medicare! 🙏`;
    }
    if (templateName === 'cod_risk_alert') {
        messageBody = `⚠️ *COD Balance Alert — Swastik Medicare*\n\nHello *${variables[0]}*,\n\nYour unremitted COD balance has crossed ₹${variables[1]}.\n\n*Action Required:* Please deposit the cash balance or contact our support team to avoid settlement holds.\n\n📞 Support: https://www.swastikmed.online/en/support`;
    }
...
export const WhatsAppTriggers = {
    ...
    // Settlement batch created — notify retailer
