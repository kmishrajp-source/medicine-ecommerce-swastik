...
export async function sendWhatsAppMessage(to, templateName, variables) {
    ...
    if (!token) {
        console.log(`[WHATSAPP MOCK] Template: "${templateName}" → +${cleanPhone} | Variables:`, variables);
        return { success: true, status: "mock_no_token" };
    }

    // Real WATI sending
    try {
        const params = (variables || []).map((v, i) => ({ name: String(i + 1), value: String(v) }));

        const response = await fetch(
            `${WHATSAPP_API_URL}?whatsappNumber=${cleanPhone}`,
            {
                method: "POST",
                headers: {
        ...
        console.log(`[WHATSAPP WATI] Template "${templateName}" → +${cleanPhone}:`, data);
        return { success: true, data };

    } catch (error) {
        console.error(`[WHATSAPP ERROR] Failed to send "${templateName}" to +${cleanPhone}:`, error.message);
