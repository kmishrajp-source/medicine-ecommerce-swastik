...
export class SpeakToBuyAgent {
  static async processVoiceIntent(rawTranscript, userId, normalizedQuery) {
    // Use raw transcript for medicine extraction (preserves full context)
    // normalizedQuery is used as fallback only
    ...
    try {
      // Step 1: Extract medicine details from raw transcript
      const extraction = await this.extractMedicineDetails(queryForExtraction);
  ...
  static async extractMedicineDetails(query) {
    // Regex fallback: grab medicine names even without OpenAI
    const strengthMatch = query.match(/\b(\d+\.?\d*\s*(?:mg|ml|mcg|g|iu|%|tablet|tab|cap|drop)s?)\b/i);
    const quantityMatch = query.match(/\b(\d+)\s*(?:tablet|tab|strip|piece|unit|pcs)/i);
    
    if (!process.env.OPENAI_API_KEY) {
      // Simple regex fallback: remove common words and extract what's left
      const stopWords = ['i','want','need','buy','give','me','get','send','provide','please','some','the','a','an','for','of','to','and'];
      const words = query.toLowerCase().split(/\s+/).filter(w => !stopWords.includes(w) && w.length > 2);
      return {
        medicineName: words[0] ? words[0].charAt(0).toUpperCase() + words[0].slice(1) : null,
    ...
    try {
      ...
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: `You are a pharmacy assistant. Extract the medicine name, strength, and quantity from the user's request.
RULES:
- medicineName must be ONLY the drug/brand name (e.g. "Paracetamol", "Crocin", "Alprazolam", "Dolo")
- strength is the dosage like "500mg", "0.5mg", "10ml" — null if not specified
- quantity is a number — default to 1 if not specified
- ALWAYS return a medicineName if any medicine or drug word is in the query
- Respond ONLY with valid JSON: { "medicineName": "...", "strength": "...", "quantity": 1 }`
          },
      ...
      // If OpenAI still returns null, try regex fallback on the raw query
      if (!result.medicineName) {
        const stopWords = ['i','want','need','buy','give','me','get','send','provide','please','some','the','a','an','for','of','to','and','tablets','capsules','syrup','medicine'];
        const words = query.toLowerCase().split(/\s+/).filter(w => !stopWords.includes(w) && w.length > 2 && !/^\d+$/.test(w));
        result.medicineName = words[0] ? words[0].charAt(0).toUpperCase() + words[0].slice(1) : null;
      }
      return result;
    } catch (err) {
      console.error("Medicine extraction error:", err.message);
      // Regex fallback on error
      const stopWords = ['i','want','need','buy','give','me','get','send','provide','please','some'];
      const words = query.toLowerCase().split(/\s+/).filter(w => !stopWords.includes(w) && w.length > 2);
      return {
        medicineName: words[0] ? words[0].charAt(0).toUpperCase() + words[0].slice(1) : null,
        strength: strengthMatch?.[1] || null,
