...
export class LanguageIntentService {
  ...
  static async processAndRoute(rawTranscript, detectedLang, userId) {
    try {
      // 0. Direct Command Interception (e.g. from UI buttons)
      if (rawTranscript.startsWith("CONFIRM_DOCTOR_BOOKING")) {
        const payloadStr = rawTranscript.split("|")[1]?.trim();
        const { HealthcareBookingOrchestrator } = await import('../healthcare-intelligence/HealthcareBookingOrchestrator.js');
        const aiResponse = await HealthcareBookingOrchestrator.executeDirectBooking(payloadStr, userId);
        return {
          success: true,
          originalTranscript: rawTranscript,
          normalizedQuery: rawTranscript,
          detectedLang,
          intent: aiResponse.intent || "BOOKING_CONFIRMED",
          isEmergency: false,
