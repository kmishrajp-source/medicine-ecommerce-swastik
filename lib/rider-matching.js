...
import { findBestExternalProvider, dispatchToExternalProvider, fallbackToRetailerDelivery, fallbackToCustomerPickup } from "./delivery-providers";
...
export async function offerToNextBestRider(jobId) {
    ...
    if (job.offerCount >= MAX_OFFER_ATTEMPTS) {
        ...
        return await escalateDeliveryJob(job);
    }
...
/**
 * Escalate a delivery job when no Swastik riders are available.
 * Tries External Provider -> Retailer Delivery -> Customer Pickup
 */
async function escalateDeliveryJob(job) {
    console.log(`[RIDER MATCH] Escalating Delivery Job ${job.id}`);
    
    // 1. Try External Provider
    const provider = await findBestExternalProvider(job.pickupLat, job.pickupLng);
    if (provider) {
        return await dispatchToExternalProvider(provider, job);
    }
    
    // 2. Try Retailer Delivery
    const retailerFallback = await fallbackToRetailerDelivery(job);
    if (retailerFallback.success) return retailerFallback;
    
    // 3. Try Customer Pickup
    return await fallbackToCustomerPickup(job);
}
