...
import { BioHealthNavigatorAgent } from './BioHealthNavigatorAgent';
import { HealthcareBookingOrchestrator } from './HealthcareBookingOrchestrator';
...
export class UnifiedNavigationAgent {
  ...
  /**
   * Detects whether a query is a booking/scheduling intent, not just a search
   */
  static _isBookingIntent(q) {
    const bookingPhrases = [
      // English
      'book', 'schedule', 'appointment', 'consult', 'need a doctor', 'see a doctor',
      'visit doctor', 'book doctor', 'book appointment', 'find me a doctor', 'i need',
      'i want to book', 'get me a', 'arrange', 'fix appointment',
      // Hindi
      'अपॉइंटमेंट', 'डॉक्टर बुक', 'डॉक्टर चाहिए', 'बुकिंग करो', 'दिखाना है',
      // Bengali
      'অ্যাপয়েন্টমেন্ট', 'ডাক্তার বুক', 'ডাক্তার দরকার'
    ];
    const hasMedicalContext = [
      'doctor', 'physician', 'specialist', 'cardiologist', 'dermatologist', 'neurologist',
      'gynecologist', 'pediatrician', 'orthopedic', 'lung', 'heart', 'skin', 'eye', 'dental',
      'डॉक्टर', 'चिकित्सक', 'विशेषज्ञ', 'ডাক্তার', 'চিকিৎসক'
    ].some(w => q.includes(w));
    return bookingPhrases.some(p => q.includes(p)) && hasMedicalContext;
  }
  ...
  static async processRequest(query, userId) {
    ...
    try {
      ...
      // ── 0.5 BIO-HEALTH & GENOMICS ───────────────────────────────
      } else if (
        q.includes("genomic") || q.includes("genetic") || q.includes("biotech") ||
        q.includes("biomarker") || q.includes("research") || /\bdna\b/i.test(q) ||
        q.includes("study") || q.includes("trial") || /\bgenes?\b/i.test(q) ||
        // Hindi
        q.includes("जीन") || q.includes("अनुवांशिक") || q.includes("जीनोमिक्स") ||
        // Bengali
        q.includes("জিন") || q.includes("জিনোমিক্স")
      ) {
        intent = "BIO_HEALTH_ROUTING";
        agentResponse = await BioHealthNavigatorAgent.handleQuery(query, userId);

      // ── 0.8 SPEAK-TO-BOOK ENGINE ─────────────────────────────────
      } else if (this._isBookingIntent(q)) {
        intent = "BOOKING_ORCHESTRATION";
        const bookingResult = await HealthcareBookingOrchestrator.processBookingIntent(query, userId, query);
        agentResponse = {
          found: true,
          ...bookingResult
        };
