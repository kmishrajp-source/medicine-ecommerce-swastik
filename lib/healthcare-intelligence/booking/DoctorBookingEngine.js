...
export class DoctorBookingEngine {
  ...
  static async executeDirectBooking(data, userId) {
    const isTestMode = process.env.TEST_MODE === 'true' || process.env.TEST_MODE === 'TRUE';
    let bookingId;
    ...
    if (!userId) {
      return {
        ...
        message: "Please sign in to your Swastik account to confirm the booking.",
        data: { message: "Please sign in to confirm." }
      };
    }
      ...
      try {
        const appt = await prisma.appointment.create({
          data: {
            ...
            date: new Date(),
            status: "Confirmed",
            reason: "Voice Booking Confirmation"
          }
        });
        bookingId = appt.id;
      } catch (err) {
    ...
    return {
      intent: "BOOKING_CONFIRMED",
      message: `Your appointment is confirmed. \nDoctor: ${data.providerName || 'Specialist'}\nSpecialty: ${data.specialty || 'General Physician'}\nBooking number: ${bookingId}.`,
      data: { bookingId, doctorId: data.doctorId, providerName: data.providerName, status: "BOOKING_CONFIRMED" }
    };
  }
}
