...
// Map DIRECT_URL (direct connection) to DATABASE_URL for seeding
process.env.DATABASE_URL = process.env.DIRECT_URL || process.env.PRISMA_DATABASE_URL;

console.log("Database connection url chosen:", process.env.DATABASE_URL ? "Yes" : "No");
...
async function seed() {
    ...
    for (const med of medicinesToSeed) {
        ...
        if (existing) {
            console.log(`Updating ${med.name}...`);
            await prisma.product.update({
                where: { id: existing.id },
                data: med
            });
        } else {
            console.log(`Creating ${med.name}...`);
